...
namespace northguan_nsa_vue_app.Server.Controllers.Record
{
    ...
    public class TrafficController(
        ITrafficRecordService trafficRecordService,
        IAuthService authService,
        IPerformanceMonitor performanceMonitor,
        ILogger<TrafficController> logger) : ControllerBase
    {
        private readonly ITrafficRecordService _trafficRecordService = trafficRecordService;
        ...
        /// <summary>
        /// 獲取車流記錄列表
        /// 支持高級過濾：車輛數量範圍、平均速度範圍、速度狀態、城市等
        /// </summary>
        /// <param name="parameters">查詢參數</param>
        /// <returns>分頁的車流記錄列表</returns>
        [HttpGet("traffic-records")]
        [ProducesResponseType(typeof(Result<PagedResponse<TrafficRecordListResponse>>), 200)]
        [ProducesResponseType(typeof(Result<PagedResponse<TrafficRecordListResponse>>), 400)]
        [ProducesResponseType(typeof(Result<PagedResponse<TrafficRecordListResponse>>), 403)]
        [ProducesResponseType(typeof(Result<PagedResponse<TrafficRecordListResponse>>), 500)]
        public async Task<ActionResult<Result<PagedResponse<TrafficRecordListResponse>>>> GetRecordsList([FromQuery] TrafficRecordQueryParameters parameters)
        {
            using var operation = _performanceMonitor.StartOperation("GetTrafficRecords",
            ...
            try
            {
                ...
                var result = await _trafficRecordService.GetRecordsListAsync(parameters);
                ...
                _logger.LogInformation("Successfully retrieved {Count} traffic records for user {UserId}",
                    result.Data.Count, _authService.GetCurrentUserId());

                return Ok(Result<PagedResponse<TrafficRecordListResponse>>.SuccessResult(
                    result,
                    $"成功獲取 {result.Data.Count} 條記錄"));
            }
            catch (Exception ex)
            {
                await _performanceMonitor.LogErrorAsync("GetTrafficRecords", ex,
                    new Dictionary<string, object>
                    {
                        ["UserId"] = _authService.GetCurrentUserId(),
                        ["Parameters"] = parameters
                ...
                _logger.LogError(ex, "Error occurred while getting traffic records for user {UserId}",
                    _authService.GetCurrentUserId());

                return StatusCode(500, Result<PagedResponse<TrafficRecordListResponse>>.FailureResult(
        ...
        {
            using var operation = _performanceMonitor.StartOperation("GetTrafficRecordsCount");
            ...
            try
            {
                ...
                var count = await _trafficRecordService.GetRecordsCountAsync(keyword, availableStationIds);

                return Ok(Result<int>.SuccessResult(count, "成功獲取記錄總數"));
            }
            catch (Exception ex)
            {
                await _performanceMonitor.LogErrorAsync("GetTrafficRecordsCount", ex);

                _logger.LogError(ex, "Error occurred while getting traffic records count");
        ...
        /// 導出車流記錄
        /// </summary>
        [HttpGet("traffic-records-export")]
            ...
            [FromQuery] string format = "csv")
        {
            ...
            var fileName = $"traffic-records_{DateTime.Now:yyyyMMdd_HHmmss}.csv";
            ...
            return new PushStreamResult(async (stream, cancel) =>
            {
                try
                {
                    ...
                    await using (writer)
                    {
                        await writer.WriteLineAsync(string.Join(",", "設備序號", "設備名稱", "分站名稱", "城市", "ETag號碼", "速限", "車流量", "平均速度", "速度狀態", "時間"));

                        var queryParams = new TrafficRecordQueryParameters
                        {
                            Keyword = keyword,
                        ...
                        int recordCount = 0;
                        await foreach (var record in _trafficRecordService.StreamAllTrafficRecordsAsync(queryParams))
                        {
                            try
                            {
                                var line = string.Join(",",
                                    ...
                                    CsvUtils.EscapeCsvField(record.City),
                                    CsvUtils.EscapeCsvField(record.ETagNumber),
                                    record.SpeedLimit,
                                    record.VehicleCount,
                                    record.AverageSpeed.ToString(CultureInfo.InvariantCulture),
                                    CsvUtils.EscapeCsvField(record.SpeedStatus),
                ...
                {
                    _logger.LogError(ex, "Error occurred while exporting traffic records");
                }
                finally
                {
