<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="container mx-auto px-4 py-8">
      <!-- 頁面標題 -->
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">車流紀錄列表與報表下載</h1>
        <p class="text-gray-600 mt-2">查看和下載車流監控的詳細記錄</p>
      </div>
      ...
      <Card class="mb-6">
        <CardContent class="p-6">
          <div class="flex flex-wrap gap-4 items-end">
            ...
            <!-- 車速範圍篩選 -->
            <div class="w-48">
              <Label class="text-sm font-medium mb-2 block">車速範圍(km/h)</Label>
              <div class="flex gap-2">
                <Input v-model.number="minAverageSpeed" placeholder="最小" type="number" min="0" class="w-20" />
                <span class="self-center">-</span>
                <Input v-model.number="maxAverageSpeed" placeholder="最大" type="number" min="0" class="w-20" />
              </div>
            </div>

            <!-- 路況狀態篩選 -->
            <div class="w-48">
              <Label class="text-sm font-medium mb-2 block">路況狀態</Label>
              <Select v-model="selectedSpeedStatus">
                <SelectTrigger>
                  <SelectValue placeholder="選擇路況" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">全部</SelectItem>
                  <SelectItem value="正常">正常</SelectItem>
                  <SelectItem value="稍擠">稍擠</SelectItem>
                  <SelectItem value="擁擠">擁擠</SelectItem>
                  <SelectItem value="超速">超速</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <table class="w-full">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900 cursor-pointer hover:bg-gray-100"
                    @click="toggleSort('vehicleCount')">
                    <div class="flex items-center">
                      車輛數
                      <span class="ml-1">
                        <ChevronUpIcon v-if="sortBy === 'vehicleCount' && sortOrder === 'asc'" class="w-4 h-4" />
                        <ChevronDownIcon v-else-if="sortBy === 'vehicleCount' && sortOrder === 'desc'"
                          class="w-4 h-4" />
                        <ChevronsUpDownIcon v-else class="w-4 h-4 text-gray-400" />
                      </span>
                    </div>
                  ...
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900 cursor-pointer hover:bg-gray-100"
                    @click="toggleSort('averageSpeed')">
                    <div class="flex items-center">
                      平均車速
                      <span class="ml-1">
                        <ChevronUpIcon v-if="sortBy === 'averageSpeed' && sortOrder === 'asc'" class="w-4 h-4" />
                        <ChevronDownIcon v-else-if="sortBy === 'averageSpeed' && sortOrder === 'desc'"
                          class="w-4 h-4" />
                        <ChevronsUpDownIcon v-else class="w-4 h-4 text-gray-400" />
                      </span>
                    </div>
                  </th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900">速限</th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900">路況</th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900 cursor-pointer hover:bg-gray-100"
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                <tr v-else v-for="(record, index) in records" :key="record.id" class="hover:bg-gray-50">
                  ...
                  <td class="px-4 py-3 text-sm text-gray-900">
                    {{ record.speed || record.averageSpeed || 0 }} km/h
                  </td>
                  <td class="px-4 py-3 text-sm text-gray-900">
                    {{ record.speedLimit || '-' }} km/h
                  </td>
                  <td class="px-4 py-3 text-sm text-gray-900">
                    <div class="flex items-center">
                      <img
                        :src="getTrafficStatusIcon(record.status || record.speedStatus || getTrafficStatus(record.averageSpeed, record.speedLimit))"
                        :alt="record.status || record.speedStatus || getTrafficStatus(record.averageSpeed, record.speedLimit)"
                        class="w-5 h-5 mr-2" />
                      {{ record.status || record.speedStatus || getTrafficStatus(record.averageSpeed, record.speedLimit)
                      }}
                    </div>
                  </td>
                  <td class="px-4 py-3 text-sm text-gray-900">
...
const selectedSpeedStatus = ref('all')
const minAverageSpeed = ref<number>()
const maxAverageSpeed = ref<number>()
...
const search = async () => {
  ...
  try {
    ...
    const speedStatuses = selectedSpeedStatus.value === 'all' ? undefined : [selectedSpeedStatus.value]
    ...
    const response = await RecordService.getTrafficRecords({
...
const exportExcel = async () => {
  ...
  try {
    ...
    await RecordService.exportTrafficRecords({
...
const getTrafficStatusIcon = (status: string) => {
  switch (status) {
    case '擁擠':
      return '/images/icons/traffic-red.svg'
    case '稍擠':
      return '/images/icons/traffic-yellow.svg'
    case '超速':
      return '/images/icons/traffic-red.svg'
    case '正常':
      return '/images/icons/traffic-green.svg'
    default:
      return '/images/icons/traffic-green.svg'
  }
}
...
// 根據速度計算狀態
const getTrafficStatus = (averageSpeed: number, speedLimit: number) => {
  if (!averageSpeed || !speedLimit) return '正常'
  if (averageSpeed > speedLimit * 1.2) return '超速'
  if (averageSpeed < speedLimit * 0.5) return '擁擠'
  if (averageSpeed < speedLimit * 0.8) return '稍擠'
  return '正常'
}
