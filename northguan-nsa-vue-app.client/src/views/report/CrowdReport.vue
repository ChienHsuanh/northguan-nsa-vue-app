<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="container mx-auto px-4 py-8">
      <!-- 頁面標題 -->
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">人流紀錄列表與報表下載</h1>
        <p class="text-gray-600 mt-2">查看和下載人流監控的詳細記錄</p>
      </div>
      ...
      <Card class="mb-6">
        <CardContent class="p-6">
          <div class="flex flex-wrap gap-4 items-end">
            ...
            <!-- 人數範圍篩選 -->
            <div class="w-48">
              <Label class="text-sm font-medium mb-2 block">人數範圍</Label>
              <div class="flex gap-2">
                <Input v-model.number="minPeopleCount" placeholder="最小" type="number" class="w-20" />
                <span class="self-center">-</span>
                <Input v-model.number="maxPeopleCount" placeholder="最大" type="number" class="w-20" />
              </div>
            </div>
            ...
            <table class="w-full">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900">累計入園數量</th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900 cursor-pointer hover:bg-gray-100"
                    @click="toggleSort('peopleCount')">
                    <div class="flex items-center">
                      人數
                      <span class="ml-1">
                        <ChevronUpIcon v-if="sortBy === 'peopleCount' && sortOrder === 'asc'" class="w-4 h-4" />
                        <ChevronDownIcon v-else-if="sortBy === 'peopleCount' && sortOrder === 'desc'" class="w-4 h-4" />
                        <ChevronsUpDownIcon v-else class="w-4 h-4 text-gray-400" />
                      </span>
                    </div>
                  </th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-900 cursor-pointer hover:bg-gray-100"
                    @click="toggleSort('density')">
                    <div class="flex items-center">
                      擁擠程度
                      <span class="ml-1">
                        <ChevronUpIcon v-if="sortBy === 'density' && sortOrder === 'asc'" class="w-4 h-4" />
                        <ChevronDownIcon v-else-if="sortBy === 'density' && sortOrder === 'desc'" class="w-4 h-4" />
                        <ChevronsUpDownIcon v-else class="w-4 h-4 text-gray-400" />
                      </span>
                    </div>
                  </th>
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                <tr v-else v-for="(record, index) in records" :key="record.id" class="hover:bg-gray-50">
                  ...
                  <td class="px-4 py-3 text-sm text-gray-900">
                    <div class="flex items-center">
                      <img :src="getCrowdStatusIcon(record.status || getCrowdStatusText(record.density))"
                        :alt="record.status || getCrowdStatusText(record.density)" class="w-5 h-5 mr-2" />
                      {{ record.status || getCrowdStatusText(record.density) }}
                    </div>
                  </td>
                  <td class="px-4 py-3 text-sm text-gray-900">
                    {{ record.time }}
...
// Utils
import { getCrowdStatusText, getCrowdStatusIcon } from '@/utils/crowdStatusUtils'
...
const minPeopleCount = ref<number>()
const maxPeopleCount = ref<number>()
...
const search = async () => {
  ...
  try {
    ...
    const response = await RecordService.getCrowdRecords({
      startDate: startDateStr,
      endDate: endDateStr,
      stationIds,
      keyword: deviceNameKeyword.value,
      minPeopleCount: minPeopleCount.value,
      maxPeopleCount: maxPeopleCount.value,
...
const exportExcel = async () => {
  ...
  try {
    ...
    await RecordService.exportCrowdRecords({
