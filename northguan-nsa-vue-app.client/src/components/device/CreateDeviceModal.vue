<template>
  <Dialog :open="isOpen" @update:open="$emit('close')">
    <DialogContent class="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle class="text-center">新增設備</DialogTitle>
        <DialogDescription class="text-center">請填寫設備的基本資訊</DialogDescription>
      </DialogHeader>
      ...
      <form class="space-y-6" @submit.prevent>
        ...
        <div class="space-y-4">
          ...
          <FormField v-slot="{ componentField }" name="type">
            <FormItem>
              ...
              <FormControl>
                <Select v-bind="componentField" @update:model-value="handleTypeChange">
                  <SelectTrigger>
        ...
        <div v-if="submitError" class="p-4 bg-red-50 border border-red-200 rounded-md">
          <div class="flex">
            ...
            <div class="ml-3">
              <h3 class="text-sm font-medium text-red-800">新增失敗</h3>
...
const deviceSchema = toTypedSchema(z.object({
  ...
  stationId: z.coerce.number({ required_error: '請選擇所屬分站' }),
  latitude: z.coerce.number({ required_error: '請輸入緯度' }),
  longitude: z.coerce.number({ required_error: '請輸入經度' }),
  serial: z.string().min(1, '請輸入設備序號'),
  // Type-specific fields
  videoUrl: z.string().optional(),
  apiUrl: z.string().optional(),
  area: z.coerce.number().optional(),
  numberOfParking: z.coerce.number().optional(),
...
const { handleSubmit, resetForm, setFieldValue, setFieldError, values, validate } = useForm({
...
watch(() => props.isOpen, (isOpen) => {
  if (isOpen && props.defaultType) {
    ...
    setFieldValue('type', props.defaultType)
  } else if (!isOpen) {
...
const handleTypeChange = (type: string | number | boolean | null) => {
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    request.stationID = Number(values.stationId)
    ...
    await DeviceService.createDevice(request)
    ...
    if (error.name === 'DuplicateRequestError' || error.isDuplicateRequest) {
      console.log('CreateDeviceModal: 重複請求被靜默處理')
      return
    }
    ...
    if (apiError.errorCode === ERROR_CODES.VALIDATION_FAILED) {
      ...
      submitError.value = `無法新增設備: ${apiError.message}`
    }
  } finally {
