<template>
  <Dialog :open="isOpen" @update:open="$emit('close')">
    <DialogContent class="sm:max-w-[425px]">
      <DialogHeader>
        <DialogTitle class="text-center">新增分站</DialogTitle>
        <DialogDescription class="text-center">請填寫分站的基本資訊</DialogDescription>
      </DialogHeader>
...
const onSubmit = handleSubmit(async (values) => {
  try {
    const request = new CreateStationRequest()
    ...
    await StationService.createStation(request)

    resetForm()
    emit('close')
    toast.success('新增成功', '分站已成功新增')
    emit('success')
  } catch (error) {
    console.error('新增分站失敗:', error)
    toast.error('新增失敗', '無法新增分站')
  }
})
</script>